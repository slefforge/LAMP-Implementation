...
from ..pruning.utils import is_classifier
...
class GlobalLAP(VisionPruning):
    ...
    def get_classifier(self):
        return [module for module in self.model.modules() if is_classifier(module)]
...
def _look_next_score(weight, weight_next):
    ...
    if weight.dim() == 2 and weight_next.dim() == 2:
        ...
        if weight.size(0) == weight_next.size(1):
            wn_squared_sum = wn_squared.sum(dim=0)
            wn_squared_sum_mat = wn_squared_sum.view(-1, 1, 1, 1).repeat(1, weight.size(1), weight.size(2),
                                                                         weight.size(3))
