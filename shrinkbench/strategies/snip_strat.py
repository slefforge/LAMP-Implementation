...
from shrinkbench.strategies import  flatten_importances, fraction_threshold, importance_masks
import torch
...
class Snip(VisionPruning):
    ...
    def snip_importances(self):


        # allow masks to have gradient
        for m, _ in self.masked_parameters1(self.model):
        ...
        for m, p in self.masked_parameters1(self.model):
        ...
        for _, p in self.masked_parameters1(self.model):
            self.scores[id(p)].div_(norm)


    def prunable1(self, module, batchnorm, residual):
    ...
    def masks1(self, module):  #todo I believe the error is here, this does not return anything so the zip in masked_parameters does not do anything
    ...
    def masked_parameters1(self, model, bias=False, batchnorm=False, residual=False):
        r"""Returns an iterator over models prunable parameters, yielding both the
        mask and parameter tensors.
        """

        for module in filter(lambda p: self.prunable1(p, batchnorm, residual), model.modules()):
            module.register_buffer('weight_mask', torch.ones(module.weight.shape))
            if module.bias is not None:
                module.register_buffer('bias_mask', torch.ones(module.bias.shape))
            for mask, param in zip(self.masks1(module), module.parameters(recurse=False)):
